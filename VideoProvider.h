...
public:
    ...
    explicit Ffmpeg_frame(QString local_video_path){
        ...
        AVCodecParameters *codecpar_video = fmt_ctx->streams[video_stream_index]->codecpar;
        AVCodecParameters *codecpar_audio = fmt_ctx->streams[audio_stream_index]->codecpar;
...
public:
    qint64 pos;
    explicit AudioWave(Ffmpeg_frame* ff,qint64 *audio_pts,QObject* parent=nullptr):QIODevice(parent),m_ff(ff),m_audio_pts(audio_pts),pos(0){
        buf.resize(buf_size);
        // 输入参数
...
private:
    ...
    int converted = swr_convert(
        ...
        buf_real_size = av_samples_get_buffer_size(
            nullptr,
            channels,
            converted,
            AV_SAMPLE_FMT_S16,
...
private:
    ...
    void generateFrame(AVFrame *frn) {
        ...
        //     if (!fi.exists()) {
        //         qWarning() << "Failed to load image:" << filePath << "-> file does not exist!";
        //     } else if (!fi.isReadable()) {
        //         qWarning() << "Failed to load image:" << filePath << "-> file is not readable!";
        //     } else {
        //         qWarning() << "Failed to load image:" << filePath
        //                    << "-> possibly unsupported format or corrupted file!";
        //     }
        ...
        if (img.size() != image.size()) {
            img = img.scaled(image.size(), Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
        }
